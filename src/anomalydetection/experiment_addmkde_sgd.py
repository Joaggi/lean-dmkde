...
def experiment_addmkde_sgd(X_train, y_train, X_test, y_test, setting, mlflow, best=False):
    ...
    with mlflow.start_run(run_name=setting["z_run_name"]) as active_run:
        ...
        qmd.compile()
        qmd.fit(x_train, epochs=setting["z_eig_epochs"], batch_size=setting["z_batch_size"], verbose=1)
